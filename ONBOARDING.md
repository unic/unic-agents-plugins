...
- **Spec-first delivery.** Work flows `/wayfinder` (or `/grill-with-docs` when it fits in one session) → `/to-spec` → `/to-tickets` → `/archon-rollout`. `/to-tickets` publishes `ready-for-agent` tickets only after you approve its breakdown in-session — read the acceptance criteria there, because nothing re-checks them before an agent picks the work up. Grill late and dispatch soon; re-grill a ticket that has sat for more than a few days. `/to-spec` labels the spec issue the same way, and that one is not implementable: check an Issue has `## What to build` before dispatching it.
