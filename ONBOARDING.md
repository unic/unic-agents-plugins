...
- **Spec-first delivery.** Work flows `/wayfinder` (or `/grill-with-docs` when it fits in one session) → `/to-spec` → `/to-tickets` → `/archon-rollout`. `/to-tickets` publishes `ready-for-agent` tickets only after you approve its breakdown in-session — that approval is the only gate before an agent picks the work up, so read the acceptance criteria there.
