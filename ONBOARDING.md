...
- **Spec-first delivery.** Work flows `/wayfinder` (or `/grill-with-docs` when it fits in one session) → `/to-spec` → `/to-tickets` → `/archon-rollout`. `/to-tickets` publishes `ready-for-agent` tickets only after you approve its breakdown in-session — read the acceptance criteria there, since that approval is followed by a pre-dispatch audit against a named commit before an agent picks the work up. `/to-spec` labels the spec issue the same way, and that one is not implementable: check an Issue has `## What to build` before dispatching it.
