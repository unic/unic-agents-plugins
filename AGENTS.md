...
1. It removes base64 data. It removes the payload of every `data:…;base64,` URI whose payload has 80 or more characters, and keeps the media type. It also removes every run of 80 or more characters made only of letters, digits, `+` and `/` that holds a digit and either ends in `=` padding or holds a `+` right after a letter or digit. A sha512 hash, 88 characters ending in `==`, is such a run. A `+` after a `+` or a `/` does not count, because a staged diff line starts with `+` and a SvelteKit route holds `/+page`. A short term inside embedded font data identifies nobody, and every Archify diagram embeds a font. A path or URL rarely holds either, so the rule still matches a long one.
