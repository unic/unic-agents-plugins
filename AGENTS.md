...
A worktree on a branch without hooks is guarded too, but only while the main work tree is on a branch that carries them. Today `main` carries only the Archon part of `pre-push`, so with the main work tree on `main` no worktree runs the NDA commit hooks or the NDA push scan. `pnpm install` then fails with the name of each missing file, but only in a worktree whose branch carries `.githooks/set-hooks-path.mjs`. In the main work tree, `main` has no `prepare` script, so the install passes in silence. That gap closes once [#576](https://github.com/unic/unic-agents-plugins/pull/576), [#577](https://github.com/unic/unic-agents-plugins/pull/577) and [#589](https://github.com/unic/unic-agents-plugins/pull/589) reach `main`.

When the hooks end up off, `pnpm install` fails and prints why, except in the cases below. In the first three, `prepare` does not run or its path is stale. One is a moved clone, where the absolute path is stale. Another is an install with `--ignore-scripts`, or with `ignore-scripts=true` in any npmrc, where `prepare` never runs. The third is an install on a branch with no `prepare` script, such as `main` today. In each of these three, check out a branch that carries `.githooks` in the main work tree, then run `pnpm install` there without that setting.

A worktree moved by hand is not reported either. `pnpm install` skips a worktree that git marks `prunable` or whose directory is gone, prints a warning and exits 0, because `prepare` cannot see the new path. Git marks a worktree moved by hand, or one whose directory it cannot read, as `prunable`. It lists a locked worktree moved by hand as `locked` instead, and the install skips that one because its directory is gone. Such a worktree still commits from its new place and reads its own `config.worktree`, so its hooks may be off. If you still use it, run `git worktree repair <new path>`, then `pnpm install`. Otherwise run `git worktree prune`, after `git worktree unlock <old path>` for a locked one. The install also skips an entry whose old path now holds another repository. `git worktree prune` does not clear that entry, so remove its directory under `.git/worktrees` by hand.

`pnpm install` reads the value back in every other worktree of the clone, except one that git marks `prunable` or whose directory is gone. A value in the `config.worktree` of a worktree that differs from the main work tree's `.githooks` fails the install and names that worktree. `prepare` runs only at install, so any change after install can turn the hooks off with no report, for example:

- A `git config --worktree` value set in a worktree after install, or a value set by hand.
- The main work tree moves to a branch that lacks a hook file, or where `pre-commit`, `commit-msg` or `pre-push` has no executable bit. Git skips a hook without the bit and prints only a `hint:`, and `advice.ignoredHook=false` hides even that. No check covers the executable bit of `pre-commit` or `commit-msg` after install, in or outside an agent session. Since [#589](https://github.com/unic/unic-agents-plugins/pull/589) the Claude hook refuses a push from an agent session when `pre-push` is not executable. A push made outside an agent session has no such check.
...
After such a change, run `pnpm install` again in the main work tree.
...
Each guard covers what it can see, and nothing more. [ADR-0036](docs/adr/0036-split-nda-guards-by-what-each-sees.md) records why.

| Guard                                                     | Covers                                                                                                                                                                                   | Wired by                                                                                         |
| --------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ |
| `.githooks/pre-push`, which runs `.githooks/nda-push.mjs` | Every push from this clone: both ref names, every tag object on the way, and every commit the push sends: the whole commit object, headers and message, and the added lines of its patch | `core.hooksPath`, which `pnpm install` sets (see above)                                          |
| `.githooks/pre-commit`, `.githooks/commit-msg`            | Any commit in this clone, whoever makes it: the added lines of the staged diff, then the message. They are an early warning, and `pre-push` catches what they miss                       | `core.hooksPath`                                                                                 |
| `.claude/hooks/block-nda-terms.mjs`                       | What git never sees: the text of `gh` and `glab` commands and the files they name. For git, it checks only that `pre-push` will run                                                      | `.claude/settings.json`, `PreToolUse` on `Bash`. It is committed, so there is nothing to install |

**`pre-push` is the git guard that matters.** A commit made with `--no-verify`, by `am`, `cherry-pick` or `merge`, or in another clone never met the commit hooks, but it cannot reach GitHub without a push. The commits a push sends are those reachable from the local sha and from neither the remote sha nor any `refs/remotes/<remote>/*` ref. A remote-tracking ref behind the remote makes it scan more. One that holds a commit the remote does not have makes it scan less, as the gaps below say. A remote sha that is not a local commit, as after a force push over commits never fetched, is left out rather than failing the listing, which scans more. A merge commit's patch is read against its first parent. A line that deletes a ref is skipped. The NDA scan runs on every push, before the Archon guard above.

**Deleted lines do not count, in `pre-commit` or in `pre-push`.** The deleted text is already in the published history, so refusing it would block the commit that removes a term. Both read diffs with `--text --no-ext-diff --no-textconv`, and `--no-color`, so a binary file, a `-diff` attribute, a textconv filter, `diff.external` and `color.ui=always` do not replace the content they read. `pre-commit` reads the diff inside node, so a NUL byte next to a term stays a boundary. The matcher reads every text a second time with its NUL bytes removed, so a term in UTF-16 text in the ASCII range is found. Compressed content is not, as the gaps below say. `pre-push` also runs git with `--no-replace-objects`, runs `git log` with `--root` and `--encoding=UTF-8`, peels nested tags one at a time, and scans the blob or tree that a tag on a non-commit points at.

**The Claude hook runs these checks on a `Bash` command, in this order.**

1. It refuses a command whose own text holds `--no-veri` or `hookspath`, in any case, whatever the verb. `--no-veri` catches `--no-verify` and every abbreviation of it that git accepts. Git rejects `--no-v`, `--no-ve` and `--no-ver` as ambiguous, so `--no-verbose` passes. The check also covers `-c core.hooksPath=` and `GIT_CONFIG_KEY_<n>=core.hooksPath`. It also refuses `git config --get core.hooksPath` and a `grep` for the word, on purpose. To mention either in a commit message, write the message to a file and commit with `-F <file>`. An authorised use is for the maintainer to run with `!`.
2. It refuses a command with `send-pack` as a word, because git runs no `pre-push` for it. It refuses a command with `push` as a word unless this clone's `pre-push` will run. A hyphen or a dot bounds a word too, so `cat .githooks/pre-push` and `nda-push.mjs` count as a push. The session's cwd must be in this clone or one of its worktrees, `core.hooksPath` must point at the `.githooks` of the main work tree, and that directory must hold `pre-push` and `nda-push.mjs`, with `pre-push` executable where the OS has an executable bit. The refusal names the condition that failed, and offers a push from the clone after `pnpm install`, or `!`.

Only a command with `git`, `gh` or `glab` as a word anywhere goes on to the checks below, and only while the term list holds a term.

3. It refuses a `gh` or `glab` command that also holds `cd` or `pushd`, because a relative path after them resolves where the hook does not look. Name every file by its absolute path, or have the maintainer run the command with `!`.
4. For a `gh` or `glab` command, it splits the text on whitespace, quotes, backticks, `=`, `@`, `<`, `(`, `)`, `$`, `;`, `&` and `|`, and it also tries whole each quoted string, each whitespace-split token, and the part of a token after its first `=`, so a path that holds `@` or `=` is read. It reads every piece that is an existing file, resolved against the session's cwd with a leading `~/` expanded, except a file whose lowercased basename is `gh`, `glab`, `gh.exe` or `glab.exe`, which is taken for the executable. That reads `--body-file=<path>`, `-F body=@file`, `--file=<path>`, `$(<file)`, `<file`, `` `cat file` `` and a quoted path with a space. It refuses a file over 2 MB rather than skip it. Reading more than a command needs costs nothing, because the hook refuses only when it finds a term.
5. It scans the command text and every file it read for a term. So `sh -c 'gh …'`, `/usr/bin/gh` and `\gh` are read too, and for `git` a branch or tag named with a term is refused before git runs.

The Claude hook reads no staged diff and parses no commit directory. `pre-commit` sees the real staged content, whatever the command string says.

**Every guard applies one matching rule, from `.githooks/nda-match.mjs`.** `pre-commit` and `commit-msg` run it with `node`. The Claude hook and `nda-push.mjs`, which `pre-push` runs, import it. Change the rule there and nowhere else. It works in three steps:
...
3. When the text holds a NUL byte and step 2 found nothing, it runs steps 1 and 2 again on the text with every NUL byte removed. UTF-16 text in the ASCII range reads as letters with a NUL between each pair, so this finds a term in it. The first run keeps a NUL next to a term as a boundary.
...
**Every guard fails closed.** With no readable term list, a commit, a push, or a `git`, `gh` or `glab` command in an agent session is refused, and the message names the file to create. `touch` that file to opt out deliberately. An empty list turns off the term checks, but the `--no-verify`, `hookspath`, `send-pack` and push checks of the Claude hook still apply. The git hooks also refuse when `node` is not on `PATH`, and `pre-push` refuses when git cannot list the commits a push sends. The Claude hook exits 2, the only exit that blocks a `PreToolUse` call, on an empty payload, one that is not a JSON object, and an exception. `.claude/settings.json` runs it as `node "$CLAUDE_PROJECT_DIR/.claude/hooks/block-nda-terms.mjs" || exit 2`, so it runs from any cwd, and any other failure, such as a missing `node` (exit 127), also blocks. This is the opposite of the `git-guardrails` defect described below, where a missing `jq` makes the hook exit 0 and read as a successful block.
