...
Before a text goes public, check it for the three shapes the rule lets through. The first is a term inside a longer word with no case change, such as `acmesite` or `Acmesite`. The second is a term inside a `data:…;base64,` payload of 80 or more characters. The third is a term inside a run of 80 or more characters made only of letters, digits, `+` and `/`, where the run holds a digit and either `=` padding at its end or a `+` after a letter, digit or `/`. A path under a `c++` directory can be such a run.
