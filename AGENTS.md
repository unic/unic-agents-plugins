...
1. It removes base64 data. It removes the payload of every `data:…;base64,` URI whose payload has 80 or more characters, and keeps the media type. It also removes every run of 80 or more base64 characters that holds a digit and either `=` padding at its end or a `+` after a letter, digit or `/`. A sha512 hash, 88 characters ending in `==`, is such a run. A `+` at the start of a run does not count, because every staged diff line starts with one. A short term inside embedded font data identifies nobody, and every Archify diagram embeds a font. A path or URL rarely holds a `+` or ends in `=`, so the rule still matches a long one.
...
Before a text goes public, check it for the three shapes the rule lets through. The first is a term joined to a letter where the join is not a change from lower to upper case, such as `acmesite`, `Acmesite` or `ACMEsite`. An upper-case term before a capitalised word, as in `ACMESite`, is still matched. The second is a term inside a `data:…;base64,` payload of 80 or more characters. The third is a term inside a run of 80 or more characters made only of letters, digits, `+` and `/`, where the run holds a digit and either `=` padding at its end or a `+` after a letter, digit or `/`. A path under a `c++` directory can be such a run.
