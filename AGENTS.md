...
**Deleted lines do not count, in `pre-commit` or in `pre-push`.** The deleted text is already in the published history, so refusing it would block the commit that removes a term. Both read diffs with `--text --no-ext-diff --no-textconv`, so a binary file, a `-diff` attribute or `diff.external` hides nothing from them.

**The Claude hook runs five checks on a `Bash` command, in this order.**

1. It refuses a command whose own text holds `--no-verify` or `hookspath`, in any case, whatever the verb. That covers `-c core.hooksPath=` and `GIT_CONFIG_KEY_<n>=core.hooksPath`. It also refuses `git config --get core.hooksPath` and a `grep` for the word, on purpose. To mention either in a commit message, write the message to a file and commit with `-F <file>`. An authorised use is for the maintainer to run with `!`.
2. It refuses a command with `push` as a word unless the session's cwd is in a repository whose `core.hooksPath` points at the `.githooks` of that repository's main work tree. A push from this clone or from one of its worktrees passes after `pnpm install`.
3. It scans the text of every command with `git`, `gh` or `glab` as a word anywhere, so `sh -c 'gh …'`, `/usr/bin/gh` and `\gh` are read too. For `git`, this refuses a branch or tag named with a term before git runs.
4. It refuses a `gh` or `glab` command that also holds `cd` or `pushd`, because a relative path after them resolves where the hook does not look. Name every file by its absolute path, or have the maintainer run the command with `!`.
5. For a `gh` or `glab` command, it splits the text on whitespace, quotes, `=`, `@`, `<`, `(`, `)` and `$`, and scans every piece that is an existing file, resolved against the session's cwd. That reads `--body-file=<path>`, `-F body=@file`, `--file=<path>`, `$(<file)` and `<file`. It refuses a file over 2 MB rather than skip it. Reading more than a command needs costs nothing, because the hook refuses only when it finds a term.
