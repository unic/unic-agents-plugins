...
| --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------ |
| `.githooks/pre-push`, which runs `.githooks/nda-push.mjs` | Every push from this clone: both ref names, every tag object on the way, and the message and added lines of every commit the push sends                            | `core.hooksPath`, which `pnpm install` sets (see above)                                          |
...
**Both guards apply one matching rule, from `.githooks/nda-match.mjs`.** `pre-commit` and `commit-msg` run it with `node`. The Claude hook and `nda-push.mjs`, which `pre-push` runs, import it. Change the rule there and nowhere else. It works in two steps:
