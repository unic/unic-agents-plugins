...
| --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------ |
| `.githooks/pre-push`, which runs `.githooks/nda-push.mjs` | Every push from this clone: both ref names, an annotated tag's object, and the message and added lines of every commit the push sends                              | `core.hooksPath`, which `pnpm install` sets (see above)                                          |
...
**`pre-push` is the git guard that matters.** A commit made with `--no-verify`, by `am`, `cherry-pick` or `merge`, or in another clone never met the commit hooks, but it cannot reach GitHub without a push. The commits a push sends are those reachable from the local sha and from neither the remote sha nor any `refs/remotes/<remote>/*` ref. A stale remote-tracking ref makes it scan more, never less. A remote sha that is not in the local object store, as after a force push over commits never fetched, is left out rather than failing the listing, which also scans more. A merge commit's patch is read against its first parent. A line that deletes a ref is skipped. The NDA scan runs on every push, before the Archon guard above.

**Deleted lines do not count, in `pre-commit` or in `pre-push`.** The deleted text is already in the published history, so refusing it would block the commit that removes a term. Both read diffs with `--text --no-ext-diff --no-textconv`, and `--no-color`, so a binary file, a `-diff` attribute, `diff.external` or `color.ui=always` hides nothing from them. `pre-commit` reads the diff inside node, so a NUL byte next to a term stays a boundary. `pre-push` also runs git with `--no-replace-objects` and `git log` with `--root`, and it scans the blob or tree that a tag on a non-commit points at.
...
4. For a `gh` or `glab` command, it splits the text on whitespace, quotes, backticks, `=`, `@`, `<`, `(`, `)`, `$`, `;`, `&` and `|`, and it also tries each quoted string whole. It reads every piece that is an existing file, resolved against the session's cwd with a leading `~/` expanded, except the `gh` or `glab` executable itself. That reads `--body-file=<path>`, `-F body=@file`, `--file=<path>`, `$(<file)`, `<file`, `` `cat file` `` and a quoted path with a space. It refuses a file over 2 MB rather than skip it. Reading more than a command needs costs nothing, because the hook refuses only when it finds a term.
...
- A quote inside a config key, such as `git -c core.hooks""Path=…`, splits the word the Claude hook looks for. The hook does not refuse that command, and the command can switch the git hooks off.
