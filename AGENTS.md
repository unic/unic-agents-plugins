...
**Type** (kind of issue): `feature`, `bug`, `spike`, `tech-debt`, `docs`, `release`. See `docs/agents/labels.md` (auto-generated by `unic-archon-dlc`; `release` is a repo-local addition in `.archon/unic-dlc.config.json`).

**Area** (which app/package): `app:<plugin>` one per app, `pkg:<package>` one per workspace package, and `repo` for monorepo-wide / cross-cutting work. The area tier is a repo convention: hand-applied, owned by neither `unic-archon-dlc` nor Matt's skills, and a candidate for future archon adoption. See [`CONTEXT-MAP.md`](CONTEXT-MAP.md) and [ADR-0032](docs/adr/0032-label-taxonomy.md).
