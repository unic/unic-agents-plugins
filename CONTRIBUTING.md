...
4. Implement each issue with `/tdd` or `/implement`, or dispatch a chain of `ready-for-agent` issues with `/archon-rollout`. Nothing re-checks a ticket after `/to-tickets` publishes it.
