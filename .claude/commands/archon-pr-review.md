...
1. **Always pass `--from develop`.** `.archon/config.yaml` sets `worktree.baseBranch: develop`, which beats Archon's stored default branch — `main` here — and beats its auto-detection. The flag beats the file in turn, so it stays belt-and-braces. Pass it anyway and verify the fork point right after dispatch:
