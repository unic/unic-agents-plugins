...
async function main() {
	...
	if (!pageUrl || !findingFile) {
		process.stderr.write(JSON.stringify({ error: 'Usage: confluence-writer.mjs --page-url <url> --finding-file <path>' }) + '\n')
		process.exit(1)
	}
	...
	try {
		...
		process.stderr.write(JSON.stringify({ error: `Failed to read finding file: ${err instanceof Error ? err.message : String(err)}` }) + '\n')
		process.exit(1)
	}

	const creds = loadAtlassianCreds()
	if (!creds) {
		process.stderr.write(JSON.stringify({ error: 'No Atlassian credentials configured - run /unic-spec-review:setup-confluence' }) + '\n')
		process.exit(1)
	}
	...
	try {
		...
		const anchor = resolution.type === 'inline' ? { textSelection: resolution.textSelection, matchCount: resolution.matchCount } : null
		const result = await postConfluenceComment(pageId, bodyWithFooter, type, anchor, creds, { fetch: globalThis.fetch })
		process.stdout.write(JSON.stringify({ id: result.id, created: result.created, type, reason: resolution.type === 'footer' ? resolution.reason : null }) + '\n')
		process.exit(0)
	} catch (err) {
