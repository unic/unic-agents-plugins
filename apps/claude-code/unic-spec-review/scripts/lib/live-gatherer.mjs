...
/**
 * Validate that parsed CLI input is an array of observations. Throws a TypeError
 ...
 * @param {unknown} raw
 * @returns {LivePageObservation[]}
 */
export function asObservationsArray(raw) {
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
	...
	let observations
	try {
		observations = asObservationsArray(JSON.parse(readFileSync(process.argv[inputFlag + 1], 'utf8')))
	} catch (err) {
