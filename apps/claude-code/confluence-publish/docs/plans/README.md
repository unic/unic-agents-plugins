...
- **Dep versioning**: `devDependencies` go through the `catalog:` section in `pnpm-workspace.yaml`, pinned exactly. Never add `^` or `~` ranges. To add/bump a dev-only dep that uses the catalog: edit the catalog entry only.
