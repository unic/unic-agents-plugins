...
3. Make the smallest set of edits that fully satisfies the spec's _Acceptance criteria_.
4. Run the _Verification_ steps from the spec. Fix anything that fails.
5. When the spec is fully done:
   ...
   - Bump versions per the spec's _Version impact_ line (both `.claude-plugin/plugin.json` and `marketplace.json` — see `CLAUDE.md`).
