...
| 11  | `/cleanup`              | command     | ✅ done | new repo-global operational janitor (report-first; composes `archon isolation`/`complete` + tracker); `cleanup` config block; retired legacy `unic-dlc-cleanup.yaml` + stub (content now in `/improve-architecture`); ADR-0028 (PR #TBD) |
| 12  | `/explore`              | Archon      | ▶️ next | `/prototype` NOT built (Matt's referenced skill)                                                                                                                                                                                         |
