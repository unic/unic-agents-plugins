...
| 09  | `/pr-review`            | Archon      | ✅ done | new generic fan-out workflow (7 intent-grounded aspects → synthesize → reconcile → gate → post); harvests unic-pr-review learnings, no dependency; iteration-aware re-review; `pr-review` config; ADR-0026 (PR _pending_) |
| 10  | `/improve-architecture` | skill       | ▶️ next | + ADR superseding                                                                                                                                                                                                         |
