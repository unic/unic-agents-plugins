...
| 08  | `/qa`                   | Archon      | ✅ done | ported to key-discriminated schema; e2e → coverage → UAT + merge gates (`gates.qa`, `all_done` AFK auto-merge, fail-closed merge); UAT-reject files agent-ready issues; `qa` config block; ADR-0025 (PR #TBD) |
| 09  | `/pr-review`            | Archon      | ▶️ next | new — harvest unic-pr-review learnings                                                                                                                                                                        |
