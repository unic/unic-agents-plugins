...
`docs/agents/*` are never read). The issue body follows Matt's `qa` shape (what happened / expected /
steps / blocked-by / context — no file paths, blocked-by honesty, prefer many thin issues) and carries
