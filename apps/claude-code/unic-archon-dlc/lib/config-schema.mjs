 ...
 *
 * `project.repo_ref` is deliberately NOT here. It is the host-agnostic repository reference every
 * PR-touching Box pins its `gh --repo` / `az repos --repository` call to, and a Box cancels cleanly
 * when it is absent rather than running against whatever repository the checkout infers. Promoting
 * it to mandatory belongs with the Archon 0.7.0 adoption, not with the defect fix that introduced it
 * (#289).
...
export function defaultConfig() {
	return {
		// `repo_ref` is the host-agnostic repository reference — "OWNER/REPO" (optionally
		// "HOST/OWNER/REPO") for github, the bare repository NAME for ado, whose `--repository` takes a
		// name or ID with the project supplied separately from `tracker.coords`. Optional: `null` means
		// "not pinned",
		// which every PR-touching Box treats as a cancel-worthy precondition rather than inferring the
		// repository from the checkout (#289).
		project: { name: null, repo_layout: 'single-context', branching: null, pr_strategy: null, repo_ref: null },
...
export function migrateLegacy(flat) {
	...
	if (flat.repo_ref !== undefined) project.repo_ref = flat.repo_ref
