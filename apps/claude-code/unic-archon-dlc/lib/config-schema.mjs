...
export function defaultConfig() {
	return {
		// `pr_strategy` is inert since #389 — no Box reads it, the host's branch policy decides a merge
		// style, and `/setup` still asks for it. Emitted so an existing config round-trips unchanged.
...
/**
 * Read a value at a dotted path (e.g. `project.branching`) from a nested object.
 ...
 *
 * A config that still carries a `tracker` block, a `classification.labels` map or a
 * `project.pr_strategy` is accepted and those keys are ignored: nothing reads them any more, and
 * rejecting a key an operator was once asked for would break a working install for no gain.
