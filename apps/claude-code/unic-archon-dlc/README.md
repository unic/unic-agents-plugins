...
| `build.coverage_threshold`                               | `null`                 | number (0–100) or `null`                      | Minimum % coverage for `/build`, and the fallback `/qa` uses when `qa.coverage_threshold` is unset. `null` no longer skips: `/build`'s `verification` runs coverage only when a threshold is set, and reports nothing about it otherwise (ADR-0037 §5)                        |
...
| `sdlc_needs.install`                                     | `null`                 | shell command string                          | Installs this project's dependencies. `/build`, `/qa` and `/pr-review` each run it once, at `bootstrap`; `/explore` does not, because it runs no check to prepare for                                                                                                         |
