...
| `build.coverage_threshold`                               | `null`                 | number (0–100) or `null`                      | Minimum % coverage. `null` no longer skips. In `/qa`, `coverage-gate` wants a figure always, so a null threshold there is `unresolved`; in `/build`, `verification` runs coverage only when a threshold is set and reports nothing about it otherwise (ADR-0037 §5) |
| `qa.coverage_threshold`                                  | `null`                 | number (0–100) or `null`                      | `/qa`'s own minimum % coverage, falling back to `build.coverage_threshold` when unset. Lets the QA gate hold a different bar from the build's                                                                                                                       |
