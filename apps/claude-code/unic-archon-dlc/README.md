...
| `project.repo_ref`                                       | asked                  | `OWNER/REPO` · `HOST/OWNER/REPO` (github) · repository name (ado) | Host-agnostic repository every PR-touching Box pins its `gh --repo` / `az repos --repository` call to, so a fork clone cannot open a PR against its upstream parent (#289). Unset → the four Archon Boxes **cancel**; `/specs` and `/tickets` **warn and ask**, because a command has a live human to ask and a node does not ([ADR-0017](docs/adr/0017-hitl-afk-gates.md)) |
