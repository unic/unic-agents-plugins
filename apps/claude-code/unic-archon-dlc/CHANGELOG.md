...
### Added
- **The evidence gate — `/build` can no longer reach `completed` on a red tree.** `unic-dlc-build.yaml` declares `evidence_policy: { required: true }`, so Archon refuses terminal `completed` unless `evidence.json` exists in the run's `$ARTIFACTS_DIR`. The engine gates on file **presence** only, which makes whatever writes that file the real gate — so a dedicated `evidence` **script** node is the only writer, never a prompt: a prompt asked to both judge the build and certify it grades its own homework, the failure [ADR-0012](docs/adr/0012-fresh-context-red-green-separation.md) already guards against for red/green. It deletes any `evidence.json` a prior resumed attempt left behind, reads the `passed` booleans `verification` and `goals-check` now report, writes the file only when both are true, and mirrors it to `<artifacts_dir>/<slug>/evidence.json` so the certification survives `/cleanup` pruning the worktree. `open-pr` stages that mirror. See [ADR-0034](docs/adr/0034-evidence-gate-deterministic-writer.md).
