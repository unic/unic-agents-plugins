...
- **`/archon-upgrade`'s "writes nothing" claim now reads "writes nothing in this repository".** Step 6's probe is behavioural, because grepping a binary proves a string is present and not that anything reads it. It writes a config and a one-node workflow inside a throwaway repository it creates outside every clone, and touches no file in the repository under assessment. The command still has no apply mode, amends no ADR and files no issue.
