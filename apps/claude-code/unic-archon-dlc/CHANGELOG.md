...
- **No config key is mandatory.** A command stops only when `.archon/unic-dlc.config.yaml` is absent or unreadable; a config that parses is one every Box can run on, with each absent key falling back to the default now stated in the command's own Step 1 table. `project.branching` was the one entry in `MANDATORY_PATHS`, and `/build`'s bootstrap node already defaulted it — so the old refusal stopped a command over a key a Box was happy to default. `/setup` still treats it as the key that decides a `partial` config from a `full` one.
