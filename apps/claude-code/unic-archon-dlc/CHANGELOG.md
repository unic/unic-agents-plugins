...
### Breaking
- **Every Box now requires `project.repo_ref`, and no pre-0.15.1 config has it.** The new `guard-no-repo-ref` node **cancels** `/build`, `/qa`, `/pr-review` and `/explore` when the key is absent, so on upgrade all four Boxes stop at their first guard until the config carries it. The key stays optional in the schema — a mandatory leaf would make `validateConfig` reject every existing config outright — but it is mandatory in practice, which is why this is filed here rather than under Added. **To upgrade:** re-run `/unic-archon-dlc:setup` (which now asks for the key and suggests a value parsed from `GIT_REMOTE`), or add it by hand under `project:` in `.archon/unic-dlc.config.yaml`. `/specs` and `/tickets` warn and ask instead of cancelling, because a command has a live human to ask and an Archon node does not ([ADR-0017](docs/adr/0017-hitl-afk-gates.md)).

### Added
- **`project.repo_ref` — an optional, host-agnostic repository reference every PR-touching Box pins its CLI call to.** `"OWNER/REPO"` (or `"HOST/OWNER/REPO"`) for github, and the bare repository **name** for ado — `az repos --repository` takes a name or ID with the project supplied separately from `tracker.coords`, so a `"PROJECT/REPO"` pair cannot be passed to it, which is why no node is allowed to split the value. Each Box's `bootstrap` node reads it and emits it verbatim as `repo_ref`, and downstream nodes pass it through the flag their host actually takes — `gh --repo`, `az repos --repository` where the subcommand has one, the reference written into the path for `gh api`, which has no `--repo` flag. The key stays **optional** and is not a mandatory config path: promoting it belongs with the Archon 0.7.0 adoption, and a mandatory leaf would invalidate every existing Consumer config on upgrade. A new `guard-no-repo-ref` node in all four Box YAMLs **cancels** the run when the key is absent, naming the key and `/unic-archon-dlc:setup` — expected precondition failures cancel, they do not fail ([ADR-0011](docs/adr/0011-archon-schema-target.md)).
...
### Fixed
- **No Box stages blindly any more (#289).** `/explore`'s `preserve-spike` ran `git add -A` and `/build`'s `open-pr` said "stage everything changed by the build". A Box runs in an isolated worktree that also carries the operator's copied `.archon/` directory, scratch files from earlier nodes, and a `build-state.json` the loop rewrites on every one of up to 60 iterations — so "everything changed" is not the change, and a blind add lands scratch and PR-body artefacts in the PR. Both nodes now stage an explicit, named list and prove it with `git status --porcelain` before committing; the loop's RED and GREEN commits name the test or implementation files alone. Archon 0.7.0 removed the same verb from its own bundled command defaults ([coleam00/Archon#2361](https://github.com/coleam00/Archon/pull/2361), issue #2229).
- **`build-state.json` reaches the PR, committed once at `open-pr`.** It is the proof of [ADR-0012](docs/adr/0012-fresh-context-red-green-separation.md)'s anti-cheat claim — `red_exit`, `red_unexpected_pass`, the per-slice phase order — and until now that proof died with the worktree when `/cleanup` pruned it. Both loop phases write it and explicitly do **not** stage it, so the history carries one coherent snapshot instead of up to 60 partial rewrites of one file.
- **Every committing node carries the deny list inline, in its own prompt.** Never `pr-body.md`, `*.tmp.md`, `*.scratch.md`, anything under `$ARTIFACTS_DIR` (Archon's per-run directory, which resolves *outside* the repo tree and is not the config's `artifacts_dir`), or anything under `.archon/`. Stated in each prompt rather than only as doctrine: a Box node is self-contained and imports nothing from the Plugin ([ADR-0023](docs/adr/0023-build-generic-red-green-refactor-loop.md) §5), so `AGENTS.md` is invisible to a running node.
- **No `gh` or `az` call infers its repository from the checkout.** `/build`'s `open-pr` and `implement-review-precheck`, `/qa`'s `verify-pr-base`, `merge` and UAT finding-capture, `/pr-review`'s `prep` and `post`, `/explore`'s `spike-ticket`, and both gates of `commands/specs.md` and `commands/tickets.md` all pin the repository to `project.repo_ref`. In a fork clone an unpinned CLI resolves to the upstream *parent*, so a PR opened, a review posted or a defect filed landed in the wrong repository. `/pr-review` is the sharpest case: it holds the only review-posting authority in the lifecycle, so an inferred repository meant a whole review — summary comment and every inline thread — on a stranger's PR.
- **`commands/specs.md` no longer stages the `docs/adr/` directory.** A directory add is not `git add -A`, but it sweeps in every unrelated in-flight ADR just the same. Each new ADR is named individually, and the `stage-only` gate path — the one that stages without committing, leaving a human to discover the extras in their own commit — stages exactly as the `open-pr` path does.
- **`/pr-review`'s `prep` node now actually waits on `guard-no-repo-ref`.** Both declared `depends_on: [bootstrap]` — the same tier the DAG runs concurrently elsewhere — so `prep`'s own prompt claim that "REPO_REF is guaranteed non-empty here: `guard-no-repo-ref` cancelled the run otherwise" was not backed by an edge. `prep` now lists `[bootstrap, guard-no-repo-ref]`. The precedent is `post`, `verify-pr-base` and `merge`, which carry a real edge over a conditionally-skipped gate — **not** `synthesize` in `/explore`, whose four upstream nodes carry no `when:` at all, which is why it needs no `trigger_rule` and why copying its shape here would reintroduce the skip fixed in the entry below.
- **`box-staging-and-repo-pinning.test.mjs`'s `nodeSource()` helper no longer misroutes on a prefix id.** `merge` is a prefix of `merge-gate`, and `merge-gate` is declared first in `unic-dlc-qa.yaml`, so a bare substring split on `- id: merge` returned `merge-gate`'s own body instead of `merge`'s. The split now requires the id to be terminated by a newline.
- **`/pr-review`'s `prep` node joins its new guard edge with `trigger_rule: all_done`.** The edge alone broke the happy path: `guard-no-repo-ref` carries a `when:`, so when `project.repo_ref` IS set the guard is *skipped*, and a skipped dependency propagates its skipped state under Archon's default `all_success` join — `prep` would have been skipped for every correctly-configured Consumer, taking `review` → `synthesize` → `reconcile` → `post` with it and turning `/pr-review` into a no-op. `post`, `verify-pr-base` and `merge` already carry `all_done` over their own gate edges for the same reason. A new test asserts the join for any node that depends on the guard.
- **`/build`'s `open-pr` now COMMITS and PUSHES what it stages.** The node built an explicit stage list, proved it with `git status --porcelain`, and went straight to `gh pr create` — with no `git commit` and no `git push` anywhere in it. The loop's RED and GREEN phases commit their own files, so source and tests reached the branch, but the five paths this node alone stages (`PRD.md`, `issues.json`, `report.md`, `build-state.json`, each drafted ADR) stayed staged and uncommitted and then died with the worktree. `gh pr create` exits 0 on the loop's last GREEN commit, so the PR opened, the URL printed, and the node reported success — indistinguishable from a correct run, and in AFK mode nobody was there to notice. The node now commits, confirms with `git show --stat --name-only HEAD` that `build-state.json` is in the commit (stopping if it is not), and pushes before opening the PR.
- **A pinned CLI call that errors STOPS instead of being retried unpinned.** Every pinning block gave the node a guarantee ("REPO_REF is guaranteed non-empty here") but no failure path. An autonomous node whose command errors retries, and the shortest retry that succeeds is to drop the flag — restoring #289 with the guard still green and the run still reporting success, which is the worst possible outcome for trust in the fix. All seven pinning blocks across the four Boxes now forbid the unpinned retry, and forbid reshaping `REPO_REF` to make a call pass.
- **`commands/tickets.md` no longer publishes issues before the warning that governs them.** Step 9 files one tracker issue per slice; the repo-pinning rule and the null-`REPO_REF` warning both sat in Step 10, which runs after it. On a Consumer with no `project.repo_ref`, every issue was filed against whatever repository the checkout inferred — in a fork clone, the upstream parent — and only then was the human asked whether they wanted that. The warning's own "before doing anything else" was unreachable from where it sat. Both command docs now run the check at the end of Step 1, where the config is read, and Step 9 names `--repo "<REPO_REF>"` inline. The "confirm the inferred repository" answer is now executable too: print what `gh repo view --json nameWithOwner` resolves to, repeat it in the question, then run unpinned — never `--repo "null"` or `--repo ""`.
- **Every staging rule now forbids `git add --all` and `git commit -a`/`-am`.** `--all` is the documented long form of `-A`, and `commit -a` stages every tracked modification *without ever calling `git add`*, so a rule naming only the `git add` spellings is defeated by the shorter thing a rewiring slice reaches for. The test's `BLIND_ADD` barrier matched three spellings out of six; it now covers `-A`, `--all`, clustered short flags, `.`, `:/` and `*`, and a new `BLIND_COMMIT` covers `commit -a`/`--all`. Its docstring no longer claims coverage the position check does not have.
- **The `trigger_rule: all_done` regression test reads `depends_on` as data, not as a string.** The reader matched the inline flow form (`depends_on: [a, b]`) only, so the equally valid YAML block-list form matched nothing and the node was *skipped rather than failed* — the same defect could return in a shape the test could not see. And the loop found exactly one qualifying node across all four Boxes, so three of its four assertions were zero-iteration no-ops reporting as passes. It now parses the YAML and asserts a non-vacuity floor, so a broken reader fails instead of passing.
- **A new test binds each `bootstrap`'s output contract to the guard condition that reads it.** The guard's `when:` was asserted verbatim; nothing asserted the producer half. Dropping `repo_ref_present` from `required`, or retyping it `boolean` (against which `== 'false'` never matches), leaves the guard *permanently false* — it never cancels, and the Box runs unpinned with the guard still sitting in the file looking correct. Every `$bootstrap.output.<field>` a `when:` reads must now appear in that Box's `required` list, and `repo_ref_present` must stay a string enum.
- **The guard also fires on a present-but-empty `repo_ref`.** The condition gated on `repo_ref_present`, a flag the same LLM `bootstrap` node derives from the value it also emits, with nothing cross-checking the two. `repo_ref: ""` with `repo_ref_present: "true"` validated, passed the guard, and reached every downstream node as `--repo ""`. All four guards now cancel on either signal.
- **`box-staging-and-repo-pinning.test.mjs`** gains node-scoped repo-pinning assertions for every PR-touching node (`implement-review-precheck`/`open-pr`, `spike-ticket`, `prep`/`post`, `verify-pr-base`/`merge`) — the prior whole-file check would still pass if one specific node silently lost its `--repo`/`--repository` flag — and a test that the command docs' null-`repo_ref` warning prints before `gh pr create` runs, guarding the one place this fix diverges by container type (workflows cancel, command docs warn-and-ask).
