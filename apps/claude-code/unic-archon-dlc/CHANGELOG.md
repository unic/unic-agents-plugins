...
- **`reconcile` stops letting a changed `file` block a match.** It matched on aspect + file + semantic title, so a finding anchored in one iteration and scoped in the next — two different `file` values by design — read as no match, and step 3 would then rule a live finding `fixed` and reply "✅ resolved" to it. The match is the hash first, then aspect + semantic title, with `file` as corroboration. A second one-off note joins the pre-#281 aspect-rename note: a prior iteration posted before 0.23.1 gave a scoped finding an invented path, so those match on aspect and title alone.
