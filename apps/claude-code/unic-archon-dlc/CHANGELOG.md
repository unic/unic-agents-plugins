...
- **`/pr-review`'s `prep` node now actually waits on `guard-no-repo-ref`.** Both declared `depends_on: [bootstrap]` — the same tier the DAG runs concurrently elsewhere — so `prep`'s own prompt claim that "REPO_REF is guaranteed non-empty here: `guard-no-repo-ref` cancelled the run otherwise" was not backed by an edge. `prep` now lists `[bootstrap, guard-no-repo-ref]`, matching the join pattern the codebase already uses (`synthesize` in `/explore`) wherever real ordering is required.
- **`box-staging-and-repo-pinning.test.mjs`'s `nodeSource()` helper no longer misroutes on a prefix id.** `merge` is a prefix of `merge-gate`, and `merge-gate` is declared first in `unic-dlc-qa.yaml`, so a bare substring split on `- id: merge` returned `merge-gate`'s own body instead of `merge`'s. The split now anchors on the id as a whole line.
