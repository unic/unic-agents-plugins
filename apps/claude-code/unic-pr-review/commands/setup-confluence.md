...
# unic-pr-review:setup-confluence

Guides you through creating `~/.unic-confluence.json` so the plugin can reach Confluence (and Jira, if configured later via `/unic-pr-review:setup-jira`).

## Step 1 — Check for existing env-var configuration

If `CONFLUENCE_URL`, `CONFLUENCE_USER`, and `CONFLUENCE_TOKEN` are all set, the file is not required for the plugin to work. Tell the user and offer to exit without writing. If they want to write the file anyway (e.g. for use without env vars), continue.
...
console.log(JSON.stringify({ url: c.url, username: c.username, token: '****' }, null, 2));
"
```
...
- They can run `/unic-pr-review:setup-jira` to add an optional Jira URL
