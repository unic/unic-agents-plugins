...
**Human Thread**:
An ADO PR comment Thread authored by a person — any Thread carrying neither the Iteration Marker (which marks a Bot Thread) nor a system `commentType` (which marks a System Thread). A Human Thread is **read-only context** for a Review: the Plugin may list an unresolved one as a Notice, or annotate a Finding that overlaps it, but it never posts to, replies on, resolves, or reopens a Human Thread in any Mode (ADR-0016). A Finding is never suppressed or down-ranked because a Human Thread exists — Confidence remains the sole filter (ADR-0002).
_Avoid_: review comment, discussion, note

**System Thread**:
An ADO PR Thread generated by the platform rather than a person — ref updates, votes, policy events — identified by `comments[0].commentType === "system"`. Never surfaced to the Reviewer.
_Avoid_: auto comment, event thread
...
- An unresolved **Human Thread** can produce a **Notice** or annotate an overlapping **Finding**, but is never written to (ADR-0016)
