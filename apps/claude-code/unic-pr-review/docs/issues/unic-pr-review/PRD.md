...
- All six code-analysis aspect agents maintain a **behavioural-parity contract** with Anthropic's `pr-review-toolkit`: each approximates the toolkit's analysis behaviour for its matching aspect. Three of them (the comment, error-handling, and types gates) additionally adopt the toolkit's _semantic spawn-gating_ patterns via deterministic diff-content sampling (see ADR-0008 amendment, issue #212); the other three retain their non-content-gated spawn conditions (always-on or path-based). The clean-slate doctrine holds: no code, no prompts, and no fixtures are taken from `apps/claude-code/pr-review/`; parity is by independent derivation against this PRD and the ADRs.
