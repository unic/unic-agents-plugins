...
- The Step 5a/5b/5c diff-decision logic (the `diffUnavailable` / `rawDiff` branching, including the empty-diff-vs-`changedFiles` cross-check) is expressed as agent prose in `ado-fetcher.md`, not as an extracted unit-tested helper — consistent with how the Fetcher's other branches (re-review, mode detection) are written. The accepted risk is that this branching is interpreted by the model and is not covered by automated tests, and its failure mode is a false-clean review. It is mitigated by three layers: the empty-diff cross-check (a non-empty `changedFiles` with an empty diff forces `diffUnavailable: true`), the fail-closed repo-match guard, and the downstream `review-pr.md` handling that suppresses aspect agents and emits the structural Notice whenever `diffUnavailable` is `true`. Extracting the decision table into a tested `scripts/lib/` helper is a tracked hardening follow-up, not a blocker.
