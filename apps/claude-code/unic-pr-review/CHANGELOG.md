...
- Re-review write path (issue #152, ADR-0006): `agents/re-review-coordinator.md` ("Arbiter", blue) is a new LLM-only agent that merges aspect-agent verdicts and ADO Thread state into a `{ threadActions, persistentUnaddressed, freshFindings }` plan (classifies each prior Thread as addressed/disputed/pending/obsolete and decides reply vs resolve vs reopen); it never calls `az devops invoke`
- `agents/ado-writer.md` gains a `mode: "re-review"` path (Steps 5–8): POSTs Replies to existing Threads via `git/comments`, PATCHes Thread status (`fixed`/`active`) via `git/threads`, opens new Threads for fresh Findings, and rewrites the existing Summary General Comment in place via a `git/comments` PATCH (falling back to creating a new Summary Thread when none exists, e.g. force-push). Reply bodies carry the Bot Signature footer for the current iteration via `renderFooter` — never `render-inline-comment.mjs` (which would add a severity emoji)
- `scripts/lib/notices.mjs`: `persistentUnaddressed` is now `PersistentUnaddressedEntry[]` (`{ threadId, threadUrl, title, sinceIteration }`) and renders each as `[title](threadUrl) _(since Iteration N)_`, ordered by the Coordinator's `sinceIteration` ascending
- `scripts/render-summary.mjs`: reads the `ITERATION` env var (default `1`) so re-review Summaries and Replies stamp the new iteration number in the Bot Signature footer
...
- `agents/re-review-coordinator.md`: added the `regressed` Thread Classification (a once-resolved/fixed Thread rated `ignored` again in the current diff) to both classification tables, mapping it to the `reopen` action that the Output contract and ADO Writer already implemented but no rule ever produced; also added an Error handling contract so the Coordinator emits `{ "error": "<reason>" }` (instead of an empty, falsely-successful plan) when `aspectFindings` is empty while `priorFindings` is non-empty, or `rawThreadsJson` is empty while `priorFindings` references thread IDs
- `agents/ado-writer.md` (issue #184): the reopen path (Step 5c) performed two distinct ADO writes — POST the reply prose, then PATCH the Thread status back to `active` — but collapsed both into a single `success` boolean reflecting only the PATCH, so a partial failure (reply succeeded, status PATCH failed, or vice versa) left the Thread half-applied with no way to localise which sub-op failed. A reopen now records one result entry per Thread with separate `replySuccess`/`statusSuccess` booleans (no top-level `success`): both sub-ops always run (no early abort), each parses its own `parseWriteResponse`, and `error` is `null` when both succeed, the failing sub-op's message when one fails, or both joined with `; ` when both fail. Step 6 documents an explicit best-effort-and-continue policy for fresh-Finding POST failures, and Step 8 treats a reopen as successful only when `replySuccess && statusSuccess`. `reply`/`resolve` entries are unchanged. New `tests/ado-writer-contract.test.mjs` adds prompt-contract smoke tests covering the split shape and aggregation policy
- Hardened the re-review render inputs against malformed agent-produced payloads: `scripts/render-summary.mjs` now fails loud (stderr + non-zero exit) when `FINDINGS_JSON` is a bare array instead of `{ findings, positiveObservations }` (previously every finding was silently dropped and an empty Findings section rendered with exit 0); the `ITERATION` clamp now falls back to `1` for non-positive values (`0`, negatives) as well as non-numeric and absent ones, so footers can no longer read "Iteration -3"; and `scripts/lib/notices.mjs` guards `persistentUnaddressed` with `Array.isArray`, so a non-array (e.g. a string from `NOTICES_JSON`) is ignored instead of being iterated character-by-character into a garbage block
