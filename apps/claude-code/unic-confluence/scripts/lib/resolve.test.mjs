...
describe('resolvePageId — key lookup', () => {
	before(() => {
		...
		writeFileSync(testPagesPath, JSON.stringify({ _comment: 'test', 'my-page': 11111, other: 22222 }, null, 2), 'utf8')
	})
...
describe('resolvePageId — error paths (via subprocess)', () => {
	...
	it('exits 1 for missing confluence-pages.json when using a key', () => {
		...
		const resolvePath = pathToFileURL(path.join(__dirname, 'resolve.mjs')).href
